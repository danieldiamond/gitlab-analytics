...
import netsuite_legacy.src.schema.vendor as vendor_schema
...
class Vendor:
    ...
    def extract(self):
        """
        Extract all the Vendor records from NetSuite

        Returns all the records found as a list
        """
        VendorSearchAdvanced = self.relationships_namespace.VendorSearchAdvanced
        vendor_search = VendorSearchAdvanced()

        return self.client.fetch_all_records_for_type(vendor_search)
    ...
    def extract_incremental(self, start_time=None, end_time=None, searchResult=None):
        ...
        if searchResult is None:
            ...
            return self.client.search_incremental(vendor_search)
    ...
    def transform(self, records):
        ...
        merge_transform_results(related_entities, [{'entity': Vendor, 'data': flat_records}])
