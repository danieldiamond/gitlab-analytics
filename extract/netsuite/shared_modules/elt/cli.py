...
def parser_db_conn(parser: ArgumentParser, required=True):
    ...
    parser.add_argument(
        "-S", "--schema", required=required, help="Database schema to use."
    )

    parser.add_argument(
        "-T", "--table", dest="table_name", help="Table to import the data to."
    )
    ...
    parser.add_argument(
        "-H",
        "--host",
        default=os.getenv("PG_ADDRESS", "localhost"),
        help="Database host address.",
    )

    parser.add_argument(
        "-p", "--port", default=os.getenv("PG_PORT", 5432), help="Database port."
    )

    parser.add_argument(
        "-u",
        "--user",
        default=os.getenv("PG_USERNAME", current_user),
