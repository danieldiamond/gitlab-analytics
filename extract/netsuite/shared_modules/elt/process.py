...
def create_tmp_table(db_conn, table_schema, table_name):
    ...
    schema, table, tmp_table = identifiers(table_schema, table_name, tmp_table_name)

    # Create temp table
    cursor = db_conn.cursor()
    create_table = psycopg2.sql.SQL(
        "CREATE TEMP TABLE {0} AS SELECT * FROM {1}.{2} LIMIT 0"
    ).format(tmp_table, schema, table)
...
def update_stmt(action, columns):
    ...
    if action not in {"NOTHING", "UPDATE"}:
        raise ValueError(
            "action must either be `NOTHING` or `UPDATE`: received {}".format(action)
        )
...
def csv_to_temp_table(db_conn, csv_path, *, table_schema, table_name, csv_options={}):
    with open(csv_path, "r") as csv_file:
        csv_file_to_temp_table(
            db_conn,
            csv_file,
...
def csv_file_to_temp_table(
    db_conn, csv_file, *, table_schema, table_name, csv_options={}
):
    with db_conn.cursor() as cursor:
        header = read_header(csv_file)
        tmp_table_name = create_tmp_table(db_conn, table_schema, table_name)

        schema, tmp_schema, table, tmp_table = identifiers(
            table_schema, "pg_temp", table_name, tmp_table_name
        )
        ...
        copy_query = psycopg2.sql.SQL(copy_stmt).format(
            schema=tmp_schema,
            table=tmp_table,
...
def overwrite_to_db_from_csv_file(
    db_conn, csv_file, *, table_schema, table_name, csv_options={}
):
    """
    ...
    try:
        ...
        # flush the table
        truncate_query = psycopg2.sql.SQL("TRUNCATE TABLE {}.{}").format(schema, table)

        # insert into table
        copy_query = psycopg2.sql.SQL(
            "COPY {0}.{1} ({2}) FROM STDIN WITH (%s)" % csv_options_stmt(**csv_options)
        ).format(
            schema, table, psycopg2.sql.SQL(", ").join(identifiers(*header.split(",")))
        )
...
def upsert_to_db_from_csv(
    db_conn, csv_path, *, primary_key, table_schema, table_name, csv_options={}
):
    """
...
def integrate_csv_file(
    ...
    *,
    ...
    try:
        tmp_table_name = csv_file_to_temp_table(
            db_conn,
            csv_file,
            table_schema=table_schema,
            table_name=table_name,
        ...
        query_stmt = """
                     ...
                     """ % update_stmt(
            update_action, update_columns
        )

        update_query = psycopg2.sql.SQL(query_stmt).format(
            schema=schema,
            table=table,
            columns=psycopg2.sql.SQL(", ").join(identifiers(*header.split(","))),
