...
WITH gitlab_issues AS (
  ...
  SELECT 
   issue_id,
   {{target.schema}}_staging.regexp_to_array(issue_description, '(?<=salesforce.com/)[0-9a-zA-Z]{15,18}') AS sfdc_link_array
...
, gitlab_issues_with_sfdc_accounts AS (
  ...
  INNER JOIN sfdc_accounts
    ON gitlab_issues_sfdc_id_flattened."18_sfdc_id" = sfdc_accounts.account_id
...
, gitlab_issues_with_sfdc_opportunities AS (
  ...
  INNER JOIN sfdc_opportunities
    ON gitlab_issues_sfdc_id_flattened."18_sfdc_id" = sfdc_opportunities.opportunity_id
...
, gitlab_issues_with_sfdc_leads AS (
  ...
  INNER JOIN sfdc_leads
    ON gitlab_issues_sfdc_id_flattened."18_sfdc_id" = sfdc_leads.lead_id
...
, gitlab_issues_with_sfdc_contacts AS (
  ...
  INNER JOIN {{ ref('sfdc_contact_xf')}} AS sfdc_contacts
    ON gitlab_issues_sfdc_id_flattened."18_sfdc_id" = sfdc_contacts.contact_id
)
