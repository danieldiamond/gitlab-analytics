WITH project_snapshot_monthly AS (
    ...
    SELECT
      PROJECT_SNAPSHOTS_MONTHLY.snapshot_month,
      ultimate_parent_id,
    ...
    INNER JOIN namespace_lineage_monthly
      ON namespace_lineage_monthly.namespace_id = PROJECT_SNAPSHOTS_MONTHLY.namespace_id
      AND namespace_lineage_monthly.snapshot_month = PROJECT_SNAPSHOTS_MONTHLY.snapshot_month
    GROUP BY 1, 2
    
), namespace_statistic_monthly_top_level AS (
    SELECT namespace_statistic_monthly.*
    FROM namespace_statistic_monthly
    INNER JOIN namespace_snapshots_monthly
      ON namespace_statistic_monthly.namespace_id = namespace_snapshots_monthly.namespace_id
      AND namespace_statistic_monthly.snapshot_month = namespace_snapshots_monthly.snapshot_month
...
SELECT
    ...
    IFF(namespace_snapshots_monthly.parent_id IS NULL, True, False)
                                                  AS has_parent_not,
    ...
    IFF(shared_runners_minutes_limit_enabled, monthly_minutes::VARCHAR, 0) AS limit_integer,
    CASE
...
INNER JOIN namespace_snapshots_monthly
  ON namespace_snapshots_monthly.snapshot_month = namespace_statistic_monthly_top_level.snapshot_month
  AND  namespace_snapshots_monthly.namespace_id = namespace_statistic_monthly_top_level.namespace_id
