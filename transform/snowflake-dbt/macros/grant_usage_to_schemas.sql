{%- macro grant_usage_to_schemas(schema_name, rolename=reporter) -%}
    ...
    {%- if target.name == 'prod' -%}
	    grant usage on schema {{ schema_name }} to role bi_reporter;
		grant select on all tables in schema {{ schema_name }} to role bi_reporter;
	    grant select on all views in schema {{ schema_name }} to role bi_reporter;
	    grant usage on schema {{ schema_name }}_meta to role bi_reporter;
		grant select on all tables in schema {{ schema_name }}_meta to role bi_reporter;
	    grant select on all views in schema {{ schema_name }}_meta to role bi_reporter;
  	{%- else -%} 
