{% macro retention_zuora_account_churn_history_cte_calc(the_month) %}
    ...
    current_subs_{{ the_month }} AS (
      ...
      FROM zuora_accts a
        JOIN zuora_subs s_1             ON s_1.account_id = a.account_id :: TEXT
   ...
   acct_churn_{{ the_month }} AS (
    SELECT
        a.account_name,
        a.crm_id,
        coalesce(o.account_id, a.account_id)    AS account_id,
      ...
      FROM zuora_accts a
        JOIN zuora_subs s ON s.account_id = a.account_id :: TEXT
        ...
        LEFT JOIN current_subs_{{ the_month }} o ON o.account_id :: TEXT = a.account_id:: TEXT
      ...
      GROUP BY
        a.account_name,
        a.crm_id,
        coalesce(o.account_id, a.account_id),
        o.current_mrr,
        o.current_arr,
    ...
    trueups_{{ the_month }} AS (
      ...
      FROM zuora_accts a
        ...
        JOIN zuora_rateplan r ON r.subscription_id :: TEXT = s.subscription_id
        JOIN zuora_rateplancharge c ON c.rate_plan_id :: TEXT = r.rate_plan_id :: TEXT
        JOIN acct_churn_{{ the_month }} z ON z.account_id :: TEXT = a.account_id:: TEXT
