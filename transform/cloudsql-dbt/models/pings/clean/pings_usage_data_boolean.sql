...
SELECT
     ...
     (container_scanning_jobs > 0)::int                     AS container_scanning_jobs,
     (dependency_scanning_jobs > 0)::int                    AS dependency_scanning_jobs,
     (deploy_keys > 0)::int                                 AS deploy_keys_active,
     (deployments > 0)::int                                 AS deployments_active,
     (environments > 0)::int                                AS environments_active,
     (epics > 0)::int                                       AS epics_active,
     (gcp_clusters > 0)::int                                AS gcp_clusters_active,
     (geo_nodes > 0)::int                                   AS geo_nodes,
     ...
     (ldap_group_links > 0)::int                            AS ldap_group_links,
     (ldap_keys > 0)::int                                   AS ldap_keys,
     (ldap_users > 0)::int                                  AS ldap_users,
     (lfs_objects > 0)::int                                 AS lfs_objects_active,
     (license_management_jobs > 0)::int                     AS license_management_jobs,
     ...
     (projects_mirrored_with_pipelines_enabled > 0)::int    AS projects_mirrored_with_pipelines_enabled,
     (projects_prometheus_active > 0)::int                  AS projects_prometheus_active_active,
     (projects_reporting_ci_cd_back_to_github > 0)::int     AS projects_reporting_ci_cd_back_to_github,
     (projects_slack_notifications_active > 0)::int         AS projects_slack_notifications_active,
     (projects_slack_slash_active > 0)::int                 AS projects_slack_slash_active,
     ...
     (sast_jobs > 0)::int                                   AS sast_jobs,
     (service_desk_enabled_projects > 0)::int               AS service_desk_enabled_projects,
     (service_desk_issues > 0)::int                         AS service_desk_issues_active,
