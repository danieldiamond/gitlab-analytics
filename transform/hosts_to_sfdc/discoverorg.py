...
discoverorg_cache = Table(
    "discoverorg_cache", metadata, autoload=True, autoload_with=engine
)
...
def get_dorg_token():
    ...
    data = dict(username=dorg_user, password=dorg_pass, partnerKey=dorg_key)
...
def check_discoverorg(domain):
    ...
    search_request = dict(
        companyCriteria=dict(
            queryString=domain, queryStringApplication=["EMAIL_DOMAIN"]
        )
    )
...
def update_discoverorg(domain):
    ...
    if company is None:
        ...
        dictlist = dict(
            ...
            company_index=content.get(
                "indexedAt", "1970-01-01 00:00:00.300000"
            ),  # not in dorg
